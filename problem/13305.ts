(function () {
  ...
  function solution(): void {
    ...
    const roadLength = input[1]
      .trim()
      .split(" ")
      .map((length) => parseInt(length));
    const gasFee = input[2]
      .trim()
      .split(" ")
      .map((gas) => parseInt(gas));

    let memoFee = Number.MAX_SAFE_INTEGER;
    const totalFee = roadLength.reduce((total, cur, index, roadLength) => {
      memoFee = Math.min(gasFee[index], memoFee);
      const curFee = BigInt(roadLength[index]) * BigInt(memoFee);
      return total + curFee;
    }, BigInt(0));
