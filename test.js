(function () {
    ...
    function solution() {
        ...
        var numArr = input.slice(1, input.length).map(function (timeStr) {
            var split = timeStr.split(" ");
            return [Number(split[0]), Number(split[1])];
        });
        var timeArr = numArr.sort(function (a, b) {
            if (a[1] < b[1]) {
                return -1;
            }
            else if (a[1] > b[1]) {
        ...
        var start = timeArr[0];
        var ans = timeArr.slice(1, timeArr.length).reduce(function (total, cur) {
            if (start[1] > cur[0]) {
                return total;
            }
            start = cur;
